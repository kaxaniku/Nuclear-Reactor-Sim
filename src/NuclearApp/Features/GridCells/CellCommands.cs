...
public record SetAbsorberAbsorptionLevelCommand(int ReactorGridId, int X, int Y, double AbsorptionLevelPercent) : IRequest, IRequiresValidReactor;
...
public record ConfigureSteamChannelCommand(int ReactorGridId, int X, int Y, double SteamGenerationRateMW, double PressureBar, double Quality, SteamType Type) : IRequest, IRequiresValidReactor;
public record ConfigureFuelChannelCommand(int ReactorGridId, int X, int Y, double NeutronFlux, double LocalPowerOutputMW, FuelRodStatus Status) : IRequest, IRequiresValidReactor;
