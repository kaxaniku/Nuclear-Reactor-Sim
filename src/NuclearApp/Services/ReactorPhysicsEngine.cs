...
public class ReactorPhysicsEngine : IReactorPhysicsEngine
{
        ...
        List<Cell> controlRods)
    {
        ...
        foreach (var ((x, y), cell) in cellMap)
        {
            if (cell.ColumnType == ColumnType.FuelChannel && cell.Telemetry is FuelChannelTelemetryDto fuel)
            {
                ...
                // Provide a minimum seed flux if the core is online to ignite chain reactions
                double currentThermal = Math.Max(fuel.ThermalFlux, 10.0);

                // Fission equation: Fast flux generated by thermal flux absorption
                double fissionFastFlux = currentThermal * localK;
        ...
        foreach (var ((x, y), cell) in cellMap)
        {
            ...
            // Retain local un-diffused flux (85% fast, 90% thermal stay local)
            AddDelta(newFastFlux, (x, y), telemetry.FastFlux * 0.85);
            AddDelta(newThermalFlux, (x, y), telemetry.ThermalFlux * 0.90);
            ...
            foreach (var (dx, dy) in Directions)
            {
                ...
                switch (neighborCell.ColumnType)
                {
                    ...
                    case ColumnType.Reflector:
                        // Reflectors bounce fast flux back into the current cell
    ...
    private static void ProcessConductionAndPhaseChange(
        ...
        double deltaTimeSeconds)
    {
        const double ThermalConductivity = 0.005;
            ...
            foreach (var (dx, dy) in Directions)
            {
                ...
                if (cellMap.TryGetValue(neighborPos, out var neighbor) && neighbor.Telemetry is CellTelemetry target)
                {
                    ...
                    if (tempDiff > 0.0)
                    {
                        ...
                        double sourceMass = cell.ColumnType == ColumnType.GraphiteModerator ? GraphiteMassKg : FuelMassKg;
                        double sourceCp = cell.ColumnType == ColumnType.GraphiteModerator ? GraphiteCp : FuelCp;
                        double maxSafeTransferMJ = (tempDiff * 0.25) * (sourceMass * sourceCp);

                        energyTransferredMJ = Math.Min(energyTransferredMJ, maxSafeTransferMJ);
        ...
        // 2. Apply Conduction Deltas & Execute Phase Change
        foreach (var ((x, y), cell) in cellMap)
        {
            if (cell.Telemetry is not CellTelemetry telemetry) continue;
            ...
            else
            {
                ...
                double deltaT = netEnergyMJ / (mass * cp);
