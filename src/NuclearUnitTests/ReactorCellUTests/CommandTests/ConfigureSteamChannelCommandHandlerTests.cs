...
public class ConfigureSteamChannelCommandHandlerTests
{
    private readonly Mock<IUnitOfWork> _unitOfWorkMock;
    private readonly ConfigureSteamChannelCommandHandler _handler;

    public ConfigureSteamChannelCommandHandlerTests()
    {
        _unitOfWorkMock = new Mock<IUnitOfWork>();
        _handler = new ConfigureSteamChannelCommandHandler(_unitOfWorkMock.Object);
    }

    [Fact]
    public async Task Handle_ShouldConfigureSteamChannel_WhenValidRequest()
    {
        // Arrange
        var request = new ConfigureSteamChannelCommand(1, 0, 0, SteamType.Dense, 0.8);
        ...
        // Assert
        var cell = reactorGrid.Cells.First();
    ...
    public async Task Handle_ShouldThrowInvalidOperationException_WhenInvalidTelemetryType()
    {
        ...
        var reactorGrid = new ReactorGrid
        {
            ...
            Cells = new List<Cell>
            {
                new Cell { ColumnType = ColumnType.SteamChannel, X = 0, Y = 0, Telemetry = new FuelChannelTelemetryDto() }
            }
        };
